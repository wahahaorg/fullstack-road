...
class SearchBoundaryTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        scenario = json.loads(
            (PROJECT_ROOT / "fixtures" / "scenario.json").read_text(encoding="utf-8")
        )
        cls.users = {item["id"]: item for item in scenario["users"]}
        cls.knowledge_bases = {item["id"]: item for item in scenario["knowledge_bases"]}
    ...
    def can_search(self, user_id: str, document_id: str) -> bool:
        ...
        return can_search_normally(self.users[user_id], document, knowledge_base)

    def test_company_document_is_visible_to_employee(self) -> None:
        self.assertTrue(self.can_search("user-finance-alice", "doc-travel-v2"))

    def test_team_document_is_hidden_from_other_team(self) -> None:
        self.assertFalse(self.can_search("user-finance-alice", "doc-oncall-v1"))

    def test_team_document_is_visible_to_team_member(self) -> None:
        self.assertTrue(self.can_search("user-engineering-bob", "doc-oncall-v1"))
    ...
    def test_draft_and_archived_documents_are_not_normally_searchable(self) -> None:
        ...
        self.assertFalse(self.can_search("user-admin-carol", "doc-travel-v1"))
