...
这类岗位的面试考察方式不是做题，而是**沿着简历项目往下追问**，大体三层：

1. **第一层（是什么）**：“你的 Agent 怎么实现的？”——读过教程就能答。
2. **第二层（怎么做）**：“规划节点怎么拆查询？工具调用循环怎么终止？流式事件协议怎么设计的？”——需要真的实现过。
3. **第三层（为什么）**：“输出 schema 怎么定的？解析失败为什么回退到规则？流式中途校验失败，已经发出去的内容怎么办？”——只有亲手做过取舍的人才答得出。

AI 生成的代码如果自己没有消化，第二层就开始含糊，而面试官恰恰会从你含糊的地方继续追——那里就是淘汰线。分工的判断标准因此只有一条：**面试官会不会追问到这一层**。

| 工作 | 谁来做 | 原因 |
|---|---|---|
| 任务 A/B/C 的核心逻辑（本文件） | 自己写，AI 出验收标准、答疑、review | 面试追问的深度所在，无法外包 |
| 外围工程（观测、CI、部署、迁移脚本） | AI 代劳，自己 review + 跑通 + 能讲清架构 | 不会被追问到配置行级别 |
| 教程章节与 README 边界声明 | 自己写初稿，AI 校对代码事实 | 写文档的过程就是组织面试叙事 |
