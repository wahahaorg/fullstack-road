...
**1. 预订系统怎么防「两个人同时订同一时段」？三种方案怎么选？**

- 悲观锁依赖 gap lock 和隔离级别，锁持有期跨整段业务逻辑，热门资源上排队；乐观锁把整个资源串行化，冲突多时重试风暴，不相关的时段也互相阻塞。
- 默认选唯一索引兜底：能接受固定粒度就把时间切成格子，`UNIQUE(resource_id, slot_start)` 直接 INSERT 捕 `ER_DUP_ENTRY`。核心优势是正确性不依赖代码路径——将来有人加新入口忘了加锁，数据库照样拦得住。
- 前提是冲突必须是真冲突：被驳回、已取消的预订要删掉对应的 `booking_slot` 行，而不是只改 `booking.status`。
- 别踩的坑：在 `resource` 上放 `is_booked` 布尔字段表达占用——它表达不了「什么时候被占」，必然和预订表打架，占用状态只该由预订记录推导。

**2. 自由区间的「时段不重叠」SQL 怎么写才不会漏？**

- 就一条：`A.start < B.end AND A.end > B.start`，严格小于——上一场 10:00 结束、下一场 10:00 开始不算冲突，区间语义是左闭右开。
- 最常见的错误写法 `start_at <= :s AND end_at >= :e` 只查到完全包住新区间的，漏掉部分重叠和反向包含——手工测试时看着是对的，线上才炸。
- 三件事必须一起做：`status` 参与过滤（不然驳回的预订永远占坑）、插入前校验 `start_at < end_at`（一条 end 早于 start 的脏数据能穿过所有重叠检查）、索引建 `(resource_id, start_at)`。

**3. 题目、商品扩展属性这类结构易变的数据，什么时候用 JSON 列？代价是什么？**

- 判断标准一句话：数据库会不会按它的内部字段查询。题目结构「业务上一直变、但从不按内部字段查」，加一种题型只改前端编辑器和一份 schema；要按内部字段搜索或过滤就别用。
- 代价是数据库不再替你把关：schema 用 TS 判别联合 + class-validator 的 `@ValidateNested` 在应用层强制；按题干搜索要么走 JSON 函数，要么另存一份纯文本冗余列。
- 每个元素要有稳定 id（nanoid，不要数组下标——插入一道题就把所有答卷的对应关系错开了）。
- 别踩的坑：正确答案跟着题目一起下发——答题接口必须返回剥掉 `answer` / `rubric` 的投影，别指望前端不看网络面板。

**4. 聊天系统的「在线状态」和「未读数」放在哪？**

- 不能放进程内存的 `Map`：扩到两个实例，A 连实例 1、B 连实例 2，A 发给 B 的消息在实例 1 的 Map 里找不到 B。跨实例投递用 Socket.IO 的 Redis adapter。
- 在线状态放 Redis 且必须 TTL 兜底（客户端心跳续期，key 过期即离线）——只靠 `disconnect` 事件，进程被 kill、网线拔掉都不会触发，会出现永远在线的幽灵用户。
- 未读数不做写扩散：一条消息发到 500 人群，给每人写一行已读就是写扩散 500 倍。存 `last_read_message_id`，未读数 = 比它大的消息条数，Redis 缓存计数。
- 消息 id 用有序 id（雪花 / ULID），分页走游标 `id < cursor`；`OFFSET` 在持续插入的表上会漏消息和重复。加分：发送方带 `client_msg_id` 加唯一索引，弱网重发天然去重。

**5. 目录结构按业务分还是按技术层分？Repository 要不要再包一层？**

- 按业务分模块：改一个功能只动一个文件夹，按技术层分（所有 controller 一个目录、所有 service 一个目录）要跳四个目录；`nest g resource` 的默认行为就是按业务分。
- 整洁架构那种 domain / application / infrastructure 分层大多数项目不需要；真正需要的信号是同一套业务逻辑要服务多个入口（HTTP + 定时任务 + 消息消费 + CLI）且已经因此重复过。没到那步之前，controller → service → repository 三层足够。
- TypeORM 的 `Repository` 已经是一层，默认让 service 直接用；只有复杂 QueryBuilder 被三处以上复用时才抽。

**6. MySQL 和 Elasticsearch 怎么保持数据一致？**

- 不要试图强一致：写库成功后发事件，消费者异步写 ES，接受最终一致——同步双写要么拖慢主链路，要么在 ES 故障时反过来拖垮写入。
- 两条兜底：消费者幂等（用业务 id 做 ES 文档 id，重复消费等于覆盖写），以及一个能按 `updated_at` 分批全量重刷的入口做对账。
- 别踩的坑：事件必须在事务提交之后发——事务里发事件，消费者可能读到还未提交的旧状态。

**7. 为什么这个规模不上微服务？**

- 拆服务的理由应该是负载画像、发布节奏或技术栈真的冲突了，而不是代码看起来乱。乱是模块边界没立住，拆开只会变成分布式单体。
- 边界先在单体里立住，再考虑拆——从模块化单体到 Monorepo 多 app 的工作量接近于搬目录，而拆成服务后新增的是网络、部署和分布式事务成本。
- 别踩的坑：为「将来可能换 ORM」提前抽 Repository 层——这件事基本不会发生。
