...
- 用 `ss -lntp` 看谁在监听，`ss -ant state syn-recv` / `state time-wait` 看状态堆积
- 容器里 127.0.0.1 只代表容器自身，服务间访问要用 0.0.0.0 监听加服务名访问

**5. DNS 排障从哪里入手？**

- `getent ahosts` 看当前解析器给出的结果；同一域名可能返回多个地址（轮询、IPv4/IPv6、地域 DNS），有结果不代表每个地址都可达
- 容器内外对比 `/etc/resolv.conf`，关注 TTL、搜索域，以及是否存在 AAAA 记录导致客户端优先尝试 IPv6
- 别踩的坑：不要把改 `/etc/hosts` 当生产修复——它会绕过 DNS 变更流程，制造配置漂移
