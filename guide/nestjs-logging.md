...
- Winston 默认只把 Error 的可枚举属性拿去序列化，而 `message` 和 `stack` 都不可枚举，结果日志里出现一个空对象——你知道出错了，但不知道错在哪。
- 格式链里挂上 `format.errors({ stack: true })`；手动调用时把 stack 当第二个参数传给 `logger.error`，否则堆栈会丢。
- 别踩的坑：这是集成 Winston 最常踩的坑——换了个日志库，异常排查能力反而倒退。

**2. 生产日志为什么必须是 JSON？**

- 解析日志的不是人是采集器（Filebeat、Fluent Bit、Loki、云日志服务）。文本日志只能全文检索；JSON 让 `level`、`context`、`userId`、`traceId` 成为独立字段，可以精确过滤、按字段聚合错误率、对字段配告警。
- 多行堆栈在文本格式下会被采集器拆成好几条互不相关的记录，JSON 把 `stack` 装进一个字段就不会。
- K8s 的约定是只写 stdout，轮转、归档、采集由平台负责；`DailyRotateFile` 适合单机 / PM2 部署，容器化之后只留 Console transport。
- 加分：能说出开发用 `prettyFormat` 给人看、生产用 `jsonFormat` 给机器看，同一份代码按环境切格式。

**3. 用户输错密码算 error 吗？日志级别该怎么定？**

- 不算。error 的定义是「需要有人现在去看」，输错密码是预期内的业务分支，一天出现几千次完全正常。
- 记成 error 会让「error 数量超阈值就告警」天天响，两周之后所有人静音，真正的故障来的时候没人看。用户维度的失败要么记 `warn`（连续失败可能是撞库），要么只进业务审计表和指标计数器。
- `debug` 不要靠删代码来关，靠配置——级别从环境变量读，线上排查时改配置重启就能开出细节。

**4. 为什么 CPU 使用率必须采两次样？`heapUsed` 和 `rss` 分别看什么？**

- `os.cpus()` 里的 `times` 是开机以来的累计毫秒数，直接拿 `user / total` 算出的是开机至今的平均使用率；想要瞬时值必须采两次样求差。
- `heapUsed` 看 JS 层有没有内存泄漏，看长期趋势；`rss` 是进程实际占用的物理内存，和容器 limit 比对判断离 OOMKilled 还有多远。
- `heapUsed` 平稳而 `rss` 一直涨，说明漏的不是 JS 对象，而是 Buffer、native 内存或内存碎片。
- 别踩的坑：容器里 `os.totalmem()` 和 `os.cpus().length` 返回的是宿主机的值——进程「明明还剩很多内存」却被 OOMKilled 就是这个原因，要知道自己的额度得读 cgroup。

**5. liveness 探针里为什么绝对不能查数据库？**
