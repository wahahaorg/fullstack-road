...
- 问五个问题：是否需要自己的 ID、是否独立新增修改删除查询、是否有多个属性、是否被多个对象引用、是否需要保存历史。
- 不是所有名词都成表："解析状态"通常只是文档或任务的字段；"文档"有文件名、大小、状态、上传人等属性，应该成表。
- 加分：判断完实体还要判断关系——一对一、一对多、多对多三类，一对多外键放"多"的一方，多对多用中间表。

**2. 主键为什么用自增 BIGINT 而不是手机号、订单号这类业务字段？**

- 业务字段可能变；字符串主键索引更大；业务字段有时需要脱敏或迁移。
- 主键只负责身份，业务唯一性用唯一索引兜底：`order_no VARCHAR(64) NOT NULL` 加 `UNIQUE KEY uk_order_no (order_no)`。
- 别踩的坑：主键和唯一索引不是一回事——主键保证"一行有唯一身份"，唯一索引保证"这个业务值不重复"，缺一不可。

**3. 用"先查一下存不存在"防止重复插入，为什么靠不住？**

- 并发下两个请求可能同时查到"不存在"，然后都插入成功。
- 数据库唯一索引才是兜底，代码捕获重复键错误，返回友好提示。
- 同理，"同一知识库下文档名不能重复"写成 `UNIQUE KEY uk_kb_title (knowledge_base_id, title)`，比代码里先查再插更可靠。

**4. 把角色 ID 存成 "1,2,3" 这样的字符串有什么问题？**

- 不好 JOIN、不好加约束，"查某个角色下有哪些用户"这种查询也写不出来。
- 多对多用中间表 `user_roles (user_id, role_id)`，联合主键天然防止重复关联。
