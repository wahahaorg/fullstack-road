---
title: NestJS 生产环境清单：测试、版本控制与安全加固
---
...
**1. 三层测试各自抓什么 bug？比例怎么定？**

- 单元测试把依赖全换成 mock，抓业务分支写反、边界条件、异常该抛没抛；集成测试连真库，抓 SQL 写错、关系映射错、事务边界错、唯一约束冲突；E2E 用 supertest 打真实 `AppModule`，抓全局管道没装、路由拼错、鉴权配错、序列化把敏感字段漏出去。
- 健康的比例是「单测覆盖业务分支 + 十几个 E2E 守住关键链路 + 集成只给复杂查询和事务写」。倒过来做（几乎没有单测、堆一大堆 E2E），CI 一次十几分钟，最后没人愿意跑，测试就死了。

**2. E2E 本地全绿，上线后同一个请求行为完全不一样？**

- `main.ts` 里那些 `useGlobalPipes` / `useGlobalFilters` 在 E2E 里一行都不会执行——E2E 是自己 `createNestApplication()` 起的应用，`bootstrap()` 根本没被调用。
- 两种解法：把全局配置抽成 `setupApp(app)`，`main.ts` 和测试都调它；或者全局切面改成 Provider，用 `APP_PIPE` / `APP_FILTER` 这类 token 注册——新项目首选，切面能注入依赖，测试 `imports: [AppModule]` 自动带上。
- 别踩的坑：`app.useGlobalGuards(new X())` 手动 new 出来的实例压根不在容器里，`overrideGuard` 对它静默失效，测试照样 401。

**3. 什么改动必须升 API 版本？为什么「改语义」最危险？**

- 只加不删不改语义的兼容（响应加字段、请求加可选字段）；删字段、改字段名、改字段类型必须升版本。改语义最危险：结构没变，客户端不报错，行为悄悄错到没人发现（`status` 加新枚举值、金额从元变分）。
- 载体有 URI / Header / Media Type / Custom 四种，对外 API 选 URI——出问题时一行访问日志就能看出调的是哪个版本；`VERSION_NEUTRAL` 匹配所有版本，但 URI 类型不支持它，而且路由按注册顺序匹配，NEUTRAL 放前面会把版本化路由吃掉。
- 落地形态是 DTO 分版本、Controller 分版本、Service 共享一份，不要在一个方法里写 `if (version === '2')`——两个版本会互相污染，半年后这种 if 散落十几处谁都不敢删。
- 下线是六步，顺序不能换：版本号打进访问日志和指标标签（先有数据再谈下线）→ 公告给到具体日期 → 打 `Deprecation` / `Sunset` 响应头 → 盯残余调用量、定位到还在调的那几家 → 低峰期 410 演练停服 → 正式下线并删掉兼容代码。
- 加分：版本数是乘法不是加法，同时在线的版本数控制在两个；正式下线返回 410 而不是 404——404 是「没有这个东西」，410 是「以前有现在没了」，调用方看错误码就知道去查迁移文档。

**4. 上了 JWT 还需要防 CSRF 吗？CORS 和 CSRF 是什么关系？**

- 取决于凭证放在哪：JWT 走 `Authorization` header 不需要 CSRF token——这个头必须由 JS 显式加，攻击者页面上的 JS 读不到你域名下的存储；一旦为了 `httpOnly` 把 JWT 塞进 cookie，就同时继承了 cookie 的 CSRF 风险，要配 `SameSite=Lax`、双提交 cookie 或校验 `Origin`。
- CORS 不拦请求，它只决定浏览器要不要把响应交给 JS；跨站表单 POST 是简单请求，压根不触发预检，副作用已经发生了——所以 CORS 不防 CSRF，也不是权限控制，真正的门是 Guard 里的鉴权。
- 别踩的坑：`origin: '*'` 和 `credentials: true` 不能一起用，规范直接禁止；CORS 函数白名单的正则不加 `^$` 锚点、不转义点号，`https://preview.example.com.evil.io` 就被放进来了。

**5. 限流应该按什么维度做？**

- 分层各管一段：网关挡 IP 级洪水，应用层（`@nestjs/throttler`）管业务配额——未登录接口按 IP，登录后按用户 / API key，登录接口用账号 + IP 组合并配 `blockDuration`。
- 两个前提：反代后面必须开 `trust proxy`，否则 `req.ip` 拿到的全是网关地址，一个人触发全站 429；默认存储是单进程内存，多实例部署实际额度放大成 N 倍，必须换 Redis 存储适配器。
- 别踩的坑：登录接口别用「失败 N 次硬锁账号」，那会变成拒绝服务工具（我锁死你的账号）——递增延迟 + 超阈值强制验证码是更好的形态。

**6. 集成测试的用例之间怎么隔离？覆盖率数字该怎么看？**

- 推荐组合：整个 suite 起一次 testcontainers 容器（或复用专用测试库），用例之间靠按外键逆序清表隔离；并行跑要 `--runInBand` 或每个 worker 一个库，否则互相踩。
- 事务回滚更快，但有个前置条件常被忽略：被测代码必须和测试用同一条连接，否则测试事务里写的数据被测代码根本看不见。满足方式是方法签名接受 `manager`、用 `AsyncLocalStorage` 事务上下文、或覆盖 repository token；被测代码自己再 `dataSource.transaction()` 会形成嵌套事务，要靠 SAVEPOINT——没有现成的事务上下文机制就别硬上，清表更省事。
- 覆盖率是「哪里一次都没跑到」的地图，不回答「测得好不好」；看分支覆盖别看行覆盖，门槛设在新增代码（diff coverage）上——全局阈值的典型后果是有人给 getter 补测试凑数。覆盖率上不去多半是设计问题：一个方法塞了 IO、分支和副作用，该拆方法，不是写更多 mock。
- 别踩的坑：换 SQLite 内存库最快，但方言不一样——JSON 列、时间函数、外键行为全对不上，测过的 SQL 上线照样报错。

**7. 上线前的安全加固要检查哪些？**

- helmet 一行装上响应头：CSP 防 XSS（注意会挡掉 Swagger UI 的内联脚本，页面空白先怀疑它）、HSTS 防降级、`nosniff` 防 MIME 嗅探；最需要这些头的其实是前端站点那台 nginx / CDN，因为 XSS 和点击劫持攻击的是 HTML 页面。
- 参数侧开 `ValidationPipe` 的 `whitelist`（+ `forbidNonWhitelisted`），防 mass assignment——DTO 只声明了 email / password，攻击者多传 `role: 'admin'`，`repo.save({ ...dto })` 就把它写进库了；两道防线一起上：开 whitelist，并且显式挑字段，不要铺开整个 body。
- SQL 注入靠 ORM 参数化基本免疫；仍会出事的只有三处——把值拼进条件字符串、`orderBy` 拼列名和排序方向（不能参数化，只能白名单枚举）、裸 SQL 拼变量。排序字段和分页参数是实战中最常见的注入口。
- 上线前逐条过：关 Swagger（或内网 + Basic Auth）、全局 Filter 生产只返回 code + message、堆栈只进日志、`synchronize` 永远 `false`、`.env` 不进仓库不进镜像、启动命令里不能有 `--inspect`。
- 加分：默认拒绝比默认放行安全——全局 `APP_GUARD` 默认要求鉴权，公开接口用 `@Public()` 显式开洞，这样「新加的接口忘了加鉴权」的默认结果是 401 而不是全世界可访问。
