...
**1. LEFT JOIN 的条件写在 ON 里和写在 WHERE 里，结果有什么不同？**

- `ON` 在连接阶段判断，匹配不上的左表行照样保留，右表字段填 NULL；`WHERE` 在连接完成后过滤整个结果集——那些补出来的 NULL 行不满足条件，被整体过滤掉，LEFT JOIN 退化成 INNER JOIN。
- 记法：左表条件放 WHERE，右表条件放 ON。
- 唯一该把右表条件写进 WHERE 的是反连接：故意找"右表没有匹配"的行，`LEFT JOIN ... WHERE o.id IS NULL`。
- 加分：INNER JOIN 里两种写法等价，所以这个坑只在 LEFT JOIN 上出现。

**2. IN、EXISTS、JOIN 三种写法怎么选？**

- 外表大、内表小用 IN；外表小、内表大且连接列有索引用 EXISTS（命中一条就短路）；需要拿内表的列只能 JOIN；只判断存在性用 EXISTS，比 JOIN 加 DISTINCT 省一次去重排序。
- "`IN` 一定慢"是过时经验：MySQL 5.6 之后优化器会把 IN 子查询改写成半连接，三种写法的执行计划经常趋同，判断标准只有一个——用 EXPLAIN 看，别背结论。
- 别踩的坑：`NOT IN` 的子查询结果里混进一个 NULL，整条 SQL 静默返回空集且不报错，排除类查询默认用 `NOT EXISTS`。

**3. 明明建了索引，为什么查询还是全表扫描？**

- 最常见的原因是回表太多：二级索引查到主键还要回聚簇索引取整行，回表次数多到一定程度，优化器干脆放弃索引走全表扫。
- 写法上的典型杀手：函数或运算包裹索引列、隐式类型转换、左模糊、OR 一侧没索引、不等号匹配行太多、区分度太低。
- 用 EXPLAIN 定位：type 是不是 ALL、key 是不是 NULL、Extra 里有没有 filesort；8.0.18 起可以用 EXPLAIN ANALYZE 看每一步的真实耗时。
- 别踩的坑：隐式类型转换最阴险——SQL 正常、结果也对，只是慢；JS 里 phone 是数字类型时，很容易就把数字传给了 VARCHAR 列。

**4. `LIMIT 1000000, 20` 这种深分页为什么越翻越慢，怎么优化？**

- 语义是"取出前 100 万零 20 行再丢掉 100 万行"，每一行都要回表，99% 的工作被扔掉。
- 延迟关联：子查询只取 id，靠覆盖索引在索引里完成扫描，再回表取 20 条——扫描量没减但每行变便宜，好处是不改接口协议。
- 游标分页：用上一页最后的 `(created_at, id)` 当起点，索引直接定位扫 20 行结束，翻到第几页都一样快；代价是不能跳页。
- 加分：选择标准是用户会不会真的翻到第 100 页——后台管理列表用延迟关联，信息流、消息列表用游标分页。

**5. 为什么现代应用基本不用存储过程和触发器？**

- 不是性能问题，是工程问题：逻辑活在数据库里不在 git 里，没法 diff 和 code review；没有断点、堆栈和日志框架，调试测试都难。
- 触发器是典型的"看不见的副作用"：读应用代码看到一句 UPDATE，不知道背后触发了什么；数据库也是全系统最难水平扩展的一环。
- 判断标准：业务逻辑写在应用层，数据库只负责存储和约束。例外是视图（权限隔离）、一次性的数据修复脚本，以及法务要求的强一致审计。
- 别踩的坑：级联删除不会触发子表触发器——靠触发器做审计日志又配了 `ON DELETE CASCADE`，会静默丢数据。
