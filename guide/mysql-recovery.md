...
> redo 用于 InnoDB 崩溃恢复，undo 用于回滚和 MVCC，binlog 用于复制与按时间点恢复。备份策略由 RPO/RTO 决定：全量或物理备份提供基线，连续归档 binlog 才能做 PITR。复制是可读扩展和切换基础，不是备份，因为误删和逻辑损坏也会复制过去。恢复我会先在隔离实例回放、校验业务数据，再决定导出修复还是切换，并把恢复耗时和数据丢失窗口记录下来。
