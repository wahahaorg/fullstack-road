...
这一版故意使用内存和离线模型，使数据流可以在没有外部账号的环境中被完整验证。它的限制也很清楚：没有持久化、权限、异步任务和 Agent 决策。继续阅读[第 3 章：用户、团队与检索权限](./agentic-rag-project-permissions)，在这套代码上加入企业知识库最先绕不开的用户与数据权限。
