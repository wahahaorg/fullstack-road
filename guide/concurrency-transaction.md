...
> 本篇从业务场景选择一致性方案。锁的分类、MySQL 加锁范围、MVCC、死锁排查与分布式锁边界，继续阅读 [锁机制与并发控制](./locking)。
...
`FOR UPDATE` 会对访问的索引记录加锁，具体范围与索引、查询条件和隔离级别有关。其他事务的冲突写入或锁定读可能要等你提交或回滚；普通一致性读通常仍能通过 MVCC 读取快照。
...
**1. 两个请求同时扣库存，库存只扣了一次，怎么修？**

- 根因是"读取 → 判断 → 修改"交错执行，两个请求都读到库存 1，都写回 0。
- 用条件更新 `SET stock = stock - 1 WHERE id = ? AND stock > 0`，判断和修改在一条 SQL 里完成；再检查影响行数，0 就是商品不存在或库存不足。
- 别踩的坑：前端禁用按钮只能改善体验——用户刷新、重试、脚本调用都能绕过，正确性必须在后端保证。

**2. 防重复提交，为什么用幂等键加唯一索引，而不是先查一下？**

- "先查有没有"在并发下会同时判断"没有"，然后重复创建。
- 支付单表给 `idempotency_key` 建唯一索引，同一次业务操作重复请求带同一个 key，数据库保证不会重复创建。
- 加分：MQ 消费、解析 worker 重试是同一个思路——消息不天然只消费一次，消费端要靠幂等表、唯一约束和任务状态机兜底。

**3. 悲观锁（FOR UPDATE）和乐观锁（version）分别适合什么场景？**

- FOR UPDATE 适合余额修改、复杂状态流转这类必须读取当前值再决定怎么改的场景；代价是并发下降、事务过长导致等待、加锁范围依赖索引。
- version 乐观锁适合内容编辑、审核状态、配置修改这类冲突不频繁的场景，影响行数为 0 就刷新重试。
- 加分：审核人 A 通过、审核人 B 同时驳回的互相覆盖，用状态流转条件更新 `WHERE status = 'reviewing'` 就能挡住，不一定需要 FOR UPDATE。

**4. 上了 MQ，并发问题是不是就解决了？**

- 先判断共享资源在哪一层：单进程内存用互斥锁或信号量；共享资源在 MySQL（库存、余额、审核状态）用事务、条件更新、行锁、乐观锁和唯一约束；多实例部署才引入 Redis 分布式锁、幂等键和 MQ 串行化消费。
- MQ 解决的是削峰、异步处理、解耦、失败重试和最终一致性；它不天然保证消息只消费一次、消费者之间没有并发、数据库一定更新成功。
- 消费端仍然要设计幂等表、唯一约束、任务状态机、失败重试、死信队列和补偿机制。
- 别踩的坑：把 MQ 当成并发银弹，消费端不做幂等，重试一次就重复入库一次。

**5. AI 应用里用户重复上传同一个 PDF，怎么防重复入库？**

- 文档表加 `UNIQUE(knowledge_base_id, file_sha256)`，同一知识库同一文件只入库一次；切片表加 `UNIQUE(document_id, chunk_index)`，worker 重试也不会重复写 chunk。
- 解析任务用状态机加幂等消费管理。
- 报表任务同理：按 user_id + report_type + params_hash 建唯一索引，已有 pending/running 任务直接返回，不重复创建。
