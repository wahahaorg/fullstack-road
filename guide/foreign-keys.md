...
> 外键是数据库层的引用完整性约束，能防止子表指向不存在的父记录，但不负责权限、状态流转或跨服务一致性。单库核心关系我倾向保留物理外键，默认 `RESTRICT`；只有真正从属于父记录、没有独立生命周期的数据才使用 `CASCADE`。订单、支付和审计数据通常用软删除或受控任务，避免一个父删除在事务里级联几十万行。分库分表和微服务无法建跨库外键，就用逻辑外键、事务、事件、唯一约束和对账补回约束能力。无论建不建，外键列索引、删除顺序、迁移校验和恢复演练都不能省。
