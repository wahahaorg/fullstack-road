...
> 我先划清单库事务和跨服务边界。订单与发送意图通过本地事务一起落库，dispatcher 使用短事务认领、租约恢复和发送确认；发送确认后崩溃仍可能重复，所以消费者用 inbox 唯一约束把去重与本地业务效果一起提交，再 ACK。乱序另用实体版本和状态机处理，外部副作用依赖幂等键与状态查询。跨服务失败通过可恢复的 Saga 补偿和对账闭环，并用故障注入验证这些窗口。

| 追问 | 应补充的边界 |
| --- | --- |
| Outbox 保证不丢吗？ | 先保证发送意图持久化；还依赖存储、队列配置、恢复任务与保留策略 |
| 为什么还需要 inbox？ | broker 接收与本地 sent 标记不原子，发送可能重复 |
| 分布式锁能替代吗？ | 锁协调并发，不能把多个系统的提交变为原子操作 |
| 幂等后可以随便重试吗？ | 要区分业务失败、未知结果、期限、载荷冲突与保留窗口 |
| 能不能同步调用库存？ | 可以，但超时和部分成功仍存在；需要状态查询、幂等和恢复流程 |
| 为什么不直接用分布式事务？ | 先确认参与系统是否支持同一协议，以及协调故障、阻塞、运维和延迟成本 |
