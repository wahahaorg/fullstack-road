...
- PostgreSQL 用 `FOR UPDATE SKIP LOCKED` 在一条 SQL 里原子领取：已被其他事务锁定的任务直接跳过，不等待也不重复领取
- 消息队列 ACK、唯一约束、分布式锁也能达到同样的「领取必须原子」
- 加分：能说出「即使现在只有一个 Worker，也建议把领取动作设计正确，为未来扩容留出空间」

**3. Worker 领了任务就断电，数据库里永远停在 running 怎么办？**

- 任务表加 `heartbeat_at` 字段，Worker 处理期间定期更新心跳
- 恢复程序发现心跳长时间未更新，把任务重新设为 pending 或标记 failed
- 状态机里 running → pending 的「Worker 崩溃后恢复」转移就是为这个准备的，`retry_count` / `max_retry_count` 同时兜住重试的死循环
...
- 先停止领取新任务，让手头任务跑完再退出；`SIGKILL` 不会给这个机会，所以进程崩溃后的 `running` 恢复要靠心跳租约兜底（见第 3 题）
- 任务执行中写的临时文件在任务结束的清理逻辑里删除；进程被硬杀的残留文件由 Worker 启动时的清扫流程处理
